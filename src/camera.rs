...
impl Camera {
    ...
    pub fn get_ray(&self, s: f32, t: f32, rng: &mut rngs::StdRng) -> Ray {
...
fn random_in_unit_disk(rng: &mut rngs::StdRng) -> Vec3 {
    loop {
        let p = 2.0 * Vec3::new(random(rng), random(rng), 0.0) - Vec3::from_i(1, 1, 0);
        if p.dot(&p) <= 1.0 {
            return p;
        }
    }
